...
Socket::~Socket() {
  sillyrpc::core::close(_fd);
}

void Socket::bind(const Address& local_addr) {
  sillyrpc::core::bind(_fd, local_addr.getSockAddr());
}

void Socket::listen() {
  sillyrpc::core::listen(_fd);
}

int Socket::accept(Address* peer_addr) {
  struct sockaddr_in addr;
  bzero(&addr, sizeof(addr));
  int connfd = sillyrpc::core::accept(_fd, &addr);
  if (connfd >= 0) {
